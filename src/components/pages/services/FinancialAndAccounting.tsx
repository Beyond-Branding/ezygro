...
import TechMahindraLogo from '../../../assets/images/logo.png';
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 lg:border-r lg:border-b border-gray-200">
        ...
        <div className="hidden lg:block">
          <div className="h-24 border-t border-l border-gray-200 flex items-center justify-center bg-white">
            <Image src={TechMahindraLogo} alt="Tech Mahindra Logo" className="h-18 object-contain" width={72} height={72} />
          </div>
                  ...
                  <div key={index} className={`p-4 text-center border-t border-gray-200 ${row.customHeight || ''} ${alignmentClass}`}>
                    <div className="w-full">
                      ...
                      <div className={`lg:hidden text-left ${row.type === 'check' ? 'flex justify-between items-center' : ''}`}>
                        ...
                        {row.type === 'data' && row.key ? (
                          <p className="text-gray-700 text-right">{plan.features[row.key as keyof typeof plan.features]}</p>
                        ) : row.type === 'check' && row.key ? (
                          plan.features[row.key as keyof typeof plan.features] ? <CheckIcon /> : <span>-</span>
                        ) : null}
                      </div>
                      {/* Desktop View */}
                      <div className="hidden lg:block">
                        {row.type === 'data' && row.key ? (
                          <p className="text-gray-700">{plan.features[row.key as keyof typeof plan.features]}</p>
                        ) : row.type === 'check' && row.key ? (
                          plan.features[row.key as keyof typeof plan.features] ? <CheckIcon /> : <span>-</span>
                        ) : null}
                      </div>
    ...
    <div className="w-full mx-auto overflow-x-auto">
      <div className="grid grid-cols-4 min-w-[768px] border-b border-gray-200">
        ...
        <div className="p-4 border-t border-b border-l border-r border-gray-200 text-center flex items-center justify-center h-16">
          <Image src={TechMahindraLogo} alt="Tech Mahindra Logo" className="h-12 object-contain" width={48} height={48} />
        </div>
